...
# @posts.route('/posts/<unit_price>', methods=['DELETE'])
# def get_inexpensive_posts(unit_price):
#     cursor = db.get_db().cursor()
#     cursor.execute('select post_id, post_dscrptn, unit_price, emp_id, street, city, zip_code, move_in, move_out \
#                     from customers where unit_price <= {0}'.format(unit_price))
...
#     # Construct the insert statements for each table
#     posts_update_stmt = 'INSERT INTO posts \
#         (sbless_id, post_dscrptn, unit_price, emp_id, street, city, zip_code, move_in, move_out) VALUES ('
#     posts_update_stmt += str(sbless_id) + ', "' + post_dscrptn + '", ' + str(unit_price) + ', ' + str(emp_id) + ', \
#         "' + street + '", "' + city + '", "' + zip_code + '", "' + move_in + '", "' + move_out + '")'

#     ap_units_update_stmt = 'INSERT INTO posts (kitchen, living_room, num_bedroom, num_bathroom) VALUES ("'
#     ap_units_insert_stmt += kitchen + '", "' + living_room + '", ' + str(num_bedroom) + ', ' + str(num_bathroom) + ')'

#     roommates_insert_stmt = 'INSERT INTO posts (roommate_gender, roommate_name, roommate_age) VALUES ('
#     roommates_insert_stmt += roommate_gender + ', "' + roommate_name + '", ' + str(roommate_age) + ')'

#     # # photos_insert_stmt = ''
#     # # photos_insert_stmt += ''

#     amenities_insert_stmt = 'INSERT INTO posts (roommate_gender, roommate_name, roommate_age) VALUES ('
#     amenities_insert_stmt += str(apartment_id) + ', "' + parking + '", "' + rec_center + '", "' + mail_room + '", \
#         "' + guard + '", "' + laundry + '", "' + wifi + '", "' + electricity + '", "' + hot_water + '")'
